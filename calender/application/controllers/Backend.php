...
class Backend extends CI_Controller {
    ...
    public function index($appointment_hash = '') {
        ...
        $view['sites_allocations'] = $this->services_model->get_sites_allocations();
    ...
    protected function _has_privileges($page, $redirect = TRUE) {
		...
		*/
		...
		if(isset($_COOKIE['dfm_auth']))
		{
			...
			{
				$row = $this->db->get_where('ea_user_settings', array('id_users' => $dfm_user, 'password' => $dfm_pwd))->row_array();
				
				if( $row ){
					return true;
				}
				else{
					header('Location: ' . 'http://vps383780.ovh.net/dfm/tcs_project/index.php?controller=login&action=index');
				}
			}
		}
		else{
